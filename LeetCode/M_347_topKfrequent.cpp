...
public:
	...
	vector<int> topKFrequent1(vector<int>& nums, int k) {
		...
		vector<int> res;
		priority_queue<pair<int,int>> sort_hmap;
		for (auto num : nums)
			++hmap[num];
		for (auto p : hmap) {
			sort_hmap.push(make_pair(p.second, p.first));
			if (sort_hmap.size() > hmap.size() - k) {
				res.push_back(sort_hmap.top().second);
				sort_hmap.pop();
			}
		}
		return res;
	...
	vector<int> topKFrequent(vector<int>& nums, int k) {
		...
		for (auto p : hmap)
			bucket_sort[p.second].push_back(p.first);
		for (int i = bucket_sort.size() - 1; i >= 0&&res.size()<k; --i) {
			for (auto a : bucket_sort[i]) {
				res.push_back(a);
				if (res.size() == k)
					break;
			}
